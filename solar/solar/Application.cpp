    ...
    while (!glfwWindowShouldClose(window)){
       ...
       */
       ...
       //太阳
        ...
        sun_proc.setMat4("model", model);
        sun_proc.setMat4("view", view);
        sun_proc.setMat4("projection", projection);

        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, textures[0]);
        sun = glm::rotate(sun, (GLfloat)glfwGetTime() * glm::radians(25.38f), glm::vec3(0.0f, 1.0f, 0.f));
        sun_proc.setMat4("model", sun);
        Sun.draw();
        positions[0] = app::p0;
        ...
        sphere_proc.setVec3("ambient", 0.1f, 0.1f, 0.1f);
        sphere_proc.setVec3("pointLight.diffuse", 0.98f, 0.98f, 0.98f);
        sphere_proc.setVec3("pointLight.specular", 1.0f, 1.0f, 1.0f);
        //设定衰减相关系数
        sphere_proc.setFloat("pointLight.k0", 1.f);
        sphere_proc.setFloat("pointLight.k1", 0.0014f);
        sphere_proc.setFloat("pointLight.k2", 0.000007f);
        sphere_proc.setVec3("sunLight.direction", -0.2f, -1.0f, -0.3f);
        sphere_proc.setVec3("sunLight.diffuse", 0.01f, 0.01f, 0.01f);
        sphere_proc.setVec3("sunLight.specular", 0.01f, 0.01f, 0.01f);
        sphere_proc.setVec3("viewPos", app::camera.position.x, app::camera.position.y, app::camera.position.z);
        sphere_proc.setFloat("material.shininess", 32.0f);
